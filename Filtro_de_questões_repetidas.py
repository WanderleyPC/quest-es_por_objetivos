def filtro_n_repetição(questões_novas):
    try:
        ...
        # Vai identificar quais são as novas questões que são repetidas
        ...
        for i in range(len(questões)):
            for j in range(i + 1, len(questões)):
                if similaridade[i][j] >= limiar:
                    ...
                    # Ele vai salvar a **posição** da novas questões que são repetidas
        ...
        '''
        print(novas_questões_são_repetidas)
...
""" # Mecanismo para testar o filtro de questões

import json
with open('teste.json', 'r', encoding='utf-8') as configuração:
    aaa=json.load(configuração)

filtro_n_repetição(aaa)
for i in filtro_n_repetição(aaa):
    print(i)
